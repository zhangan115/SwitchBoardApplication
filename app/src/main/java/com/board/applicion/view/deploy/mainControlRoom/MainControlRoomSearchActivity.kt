...
class MainControlRoomSearchActivity : BaseSearchActivity<MainControlRoom>() {
    ...
    override fun getQueryBuild(): QueryBuilder<MainControlRoom> {
        return databaseStore.getQueryBuilder().contains(MainControlRoom_.name, searchContentStr).equal(MainControlRoom_.status, 0)
    }
